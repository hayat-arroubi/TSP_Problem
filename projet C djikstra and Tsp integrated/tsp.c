...
void TSPUsingMatrix(int** distanceMatrix, int* path, int* fpath, long long* fsum, int flag, int n, int b, int a) {
    if (flag == n) {
        long long currentSum = 0;

        for (int i = 1; i < n; i++) {
            currentSum += distanceMatrix[path[i - 1]][path[i]];
        }
        currentSum += distanceMatrix[path[n - 1]][path[0]]; // Closing the loop
        if (currentSum < *fsum) {
            *fsum = currentSum;
            for (int i = 0; i < n; i++) {
                fpath[i] = path[i] + 1; // Convert back to 1-based indexing
            }
        }
        return;
    }
    for (int i = a; i <= b; i++) {
        ...
        path[a] = path[i];
        path[i] = temp;
        TSPUsingMatrix(distanceMatrix, path, fpath, fsum, flag + 1, n, b, a + 1);
        temp = path[a];
        path[a] = path[i];
        path[i] = temp;
    }
